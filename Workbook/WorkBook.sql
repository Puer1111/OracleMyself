...
select * from tb_class;
SELECT CLASS_NAME FROM TB_CLASS WHERE CLASS_NAME ='음악학과';
select STUDENT_NO , point , STUDENT_NAME from TB_GRADE E join TB_STUDENT using (STUDENT_NO) join TB_CLASS C ON  C.CLASS_NO = E.CLASS_NO where class_name = '음악학과';
--Q17
select * from tb_student;
select * from tb_class;
SELECT DISTINCT CLASS_NAME FROM TB_CLASS JOIN TB_STUDENT USING (DEPARTMENT_NO) WHERE STUDENT_NAME = '최경희';
SELECT STUDENT_NAME , STUDENT_ADDRESS FROM TB_STUDENT JOIN TB_CLASS USING (DEPARTMENT_NO) WHERE CLASS_NAME = (SELECT STUDENT_NAME FROM TB_STUDENT WHERE STUDENT_NAME ='최경희');
--DDL 
--Q1
...
--Q9
DESC TB_DEPARTMENT;
ALTER TABLE TB_CATEGORY ADD CONSTRAINT FK_DEPARTMENT_CATEFORY FOREIGN KEY (NAME) REFERENCES TB_CATEGORY(NAME);
--Q10
