...
-- 트리거: 방아쇠 ,연쇄반응
--특정 이벤트나 DDL,DML문장이 실행되었을떄
--자동적으로 일련의 동작(OPERATION)처리가 수행되도록 하는 데이터베이스 객체 중 하나임
--EX)회원탈퇴가 이루어진 경우 탈퇴한 회원 정보를 일정기간 저장해야 하는 경우
--EX2)데이터 변경이 있을 떄 , 조작한 데이터에 대한 로그(이력)을 남겨야 하는경우

--트리거 사용법
--CREATE TRIGGER 트리거명
--BEFORE (OR AFTER)
--DELETE(OR UPDATE OR INSERT) ON 테이블 명
-- [FOR EACH ROW]   --> 모든 행에 대해서
--BEGIN
--      (실행문)
--END;
--/
...
--EX 사원 테이블에 새로운 데이터가 들어오면 ' 신입사원이 입사하였습니다. ' 를 출력
...
END;
/
--EX2 EMPLOYEE 테이블에 급여 정보가 변경되면 전후 정보를 화면에 출력하는 트리거를 생성하시오.
CREATE OR REPLACE TRIGGER TRG_EMP_SALARY
AFTER
UPDATE ON EMPLOYEE
FOR EACH ROW
BEGIN
    IF(:OLD.SALARY != NEW.SALARY) -- UPDATE 썼다고 실행되는걸 방지
    THEN
    DBMS_OUTPUT.PUT_LINE('급여 정보가 변경 되었습니다');
    DBMS_OUTPUT.PUT_LINE('변경 전: '||:OLD.SALARY); --변경 전의 레코드
    DBMS_OUTPUT.PUT_LINE('변경 후: '||:NEW.SALARY);--변경 후의 레코드
END IF;
END;
/
UPDATE EMPLOYEE SET SALARY = 4000000 WHERE EMP_ID = '211';
...
CREATE SEQUENCE SEQ_PRODUCT_PCODE START WITH 1 NOCYCLE NOCACHE;
CREATE SEQUENCE SEQ_PRODUCTIO_IOCODE START WITH 1 NOCYCLE NOCACHE;
...
--제품 테이블 정보 입력
INSERT INTO PRODUCT VALUES (SEQ_PRODUCT_PCODE.NEXTVAL , '갤럭시 폰 ' , 'SAMSUNG ' ,2000000,DEFAULT); 
INSERT INTO PRODUCT VALUES (SEQ_PRODUCT_PCODE.NEXTVAL , '아이폰 ' , 'APPLE ' ,2500000,DEFAULT);
INSERT INTO PRODUCT VALUES (SEQ_PRODUCT_PCODE.NEXTVAL , '대륙폰 ' , '샤오미 ' ,20000,DEFAULT);
SELECT * FROM PRODUCT;

--제품 출고 테이블 INSERT - > 입고면 재고 +1 출고면 재고 -1
